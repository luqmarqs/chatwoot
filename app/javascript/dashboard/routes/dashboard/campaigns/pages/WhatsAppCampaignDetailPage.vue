...
onMounted(() => store.dispatch('whatsappBulkCampaigns/fetchRecipients', campaignId.value));
...
<template>
  <div class="flex flex-col h-full p-6 overflow-auto bg-n-surface-1">
    <div class="flex items-center gap-4 mb-6">
      ...
      <h2 class="text-lg font-semibold text-n-slate-12">{{ t('CAMPAIGN.WHATSAPP.HEADER_TITLE') }}</h2>
    </div>

    <div v-if="stats" class="grid grid-cols-5 gap-3 mb-6">
      <div class="p-3 text-center border rounded-lg border-n-weak bg-n-surface-2">
    ...
    <div v-else-if="!recipients.length" class="py-12 text-center text-n-slate-11">No recipients yet</div>
    <div v-else class="flex flex-col gap-1">
      <div v-for="r in recipients" :key="r.id" class="flex items-center justify-between px-4 py-3 border rounded-lg border-n-weak bg-n-surface-2">
        <div>
          <p class="text-sm font-medium text-n-slate-12">{{ r.phone_number }}</p>
          <p class="text-xs text-n-slate-10">{{ r.recipient_key }}</p>
        </div>
        <span :class="['px-2 py-0.5 text-xs font-medium rounded-full', statusColors[r.status] || 'bg-n-alpha-2 text-n-slate-12']">
          {{ r.status }}
        </span>
      </div>
    </div>
