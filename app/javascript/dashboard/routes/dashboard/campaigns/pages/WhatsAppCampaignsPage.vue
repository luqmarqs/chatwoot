  ...
  >
    ...
    <div v-else class="grid gap-4">
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3">
              <p class="font-semibold text-n-slate-12 truncate">{{ campaign.name }}</p>
              <span :class="['px-2 py-0.5 text-xs font-medium rounded-full', statusColors[campaign.status] || 'bg-n-alpha-2 text-n-slate-12']">
                {{ campaign.status }}
              </span>
            </div>
            <p v-if="campaign.description" class="mt-1 text-sm text-n-slate-11 truncate">{{ campaign.description }}</p>
            <div class="flex flex-wrap gap-4 mt-2 text-xs text-n-slate-10">
              <span v-if="campaign.provider_template_name">{{ campaign.provider_template_name }}</span>
              <span v-if="campaign.total_recipients">• {{ campaign.total_recipients }} recipients</span>
            </div>
          </div>
          <Button
            v-if="canSend(campaign)"
            :label="t('CAMPAIGN.WHATSAPP.CREATE.BUTTONS.CREATE')"
            color="blue"
            size="sm"
        ...
        <div v-if="campaign.status && !['draft','validating'].includes(campaign.status)" class="grid grid-cols-4 gap-3 mt-4 pt-4 border-t border-n-weak">
          <div class="text-center"><p class="text-lg font-semibold text-n-slate-12">{{ campaign.total_recipients || 0 }}</p><p class="text-xs text-n-slate-10">Total</p></div>
          <div class="text-center"><p class="text-lg font-semibold text-n-green-11">{{ campaign.succeeded_count || 0 }}</p><p class="text-xs text-n-slate-10">Enviado</p></div>
          <div class="text-center"><p class="text-lg font-semibold text-n-slate-10">{{ campaign.pending_count || 0 }}</p><p class="text-xs text-n-slate-10">Pendente</p></div>
          <div class="text-center"><p class="text-lg font-semibold text-n-red-11">{{ campaign.failed_count || 0 }}</p><p class="text-xs text-n-slate-10">Falha</p></div>
        </div>
      </article>
    </div>
  </CampaignLayout>
